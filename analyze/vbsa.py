...
def first_order_PSK(model, Y):
    ...
    # Number of process and process models
    npros = len(model.frames['names'])
    nmodels = [len(model.frames['options'][i]) for i in range(npros)]
    ...
    for ipros in range(npros):
        ...
        tmp_Y = tmp_Y.reshape((nmodels[ipros], N, int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))
        
        # compute the model weithts for process ~K and K
        PM_K = model.frames['weights'][ipros]
        weights = model.frames['weights'].copy()
        weights.pop(ipros)
        PM_non_K = [np.product(np.array((list(i)))) for i in itertools.product(*weights)]
        
        # compute the first-order process sensitivty index
        E_tb_Y = np.zeros((nmodels[ipros], N, int(np.product(nmodels) / nmodels[ipros])))
        E2_tb_Y = np.zeros((nmodels[ipros], N, int(np.product(nmodels) / nmodels[ipros])))
        E_b_Y  = np.zeros((nmodels[ipros], N))   
        E_b_Y2  = np.zeros((nmodels[ipros], N))   
        E2_b_Y  = np.zeros((nmodels[ipros], N))
        E_ta_Y = np.zeros(nmodels[ipros])
        E_ta_Y2 = np.zeros(nmodels[ipros])
        E2_ta_Y = np.zeros(nmodels[ipros])
                
                           
        for i in range(nmodels[ipros]):
            for j in range(N):
                for k in range(int(np.product(nmodels) / nmodels[ipros])):
...
def total_effect_PSTK(model, Y):
    ...
    for ipros in range(npros):
        ...
        # compute the first-order process sensitivty index
        E_tb_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1), nmodels[ipros]))
        E2_tb_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1), nmodels[ipros]))
        E_b_Y  = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))   
        E_b_Y2  = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))   
        E2_b_Y  = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))
        E_ta_Y = np.zeros(int(np.product(nmodels) / nmodels[ipros]))
        E_ta_Y2 = np.zeros(int(np.product(nmodels) / nmodels[ipros]))
        E2_ta_Y = np.zeros(int(np.product(nmodels) / nmodels[ipros]))
                
                           
        for i in range(int(np.product(nmodels) / nmodels[ipros])):
            for j in range(N**(npros - 1)):
                for k in range(nmodels[ipros]):
