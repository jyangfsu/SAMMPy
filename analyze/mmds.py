...
def mmds_mean_var(model, Y):
    ...
    for ipros in range(npros):
        ...
        tmp_D = np.zeros((N, N))
        D_tc_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1), nmodels[ipros], nmodels[ipros]))
        D2_tc_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1), nmodels[ipros], nmodels[ipros])) 
        D_c_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1),  nmodels[ipros]))
        D2_c_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1), nmodels[ipros]))
        D_tb_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))
        D2_tb_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros]), N**(npros - 1)))
        D_b_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros])))
        D2_b_Y = np.zeros((int(np.product(nmodels) / nmodels[ipros])))
                   
        for i in range(int(np.product(nmodels) / nmodels[ipros])):
            for j in range(N**(npros - 1)):
                for k1 in range(nmodels[ipros]):
                    for k2 in range(nmodels[ipros]):
                        for l1 in range(N):
                            for l2 in range(N):
